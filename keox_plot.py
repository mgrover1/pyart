...
ax.plot(d["new_p90"], z, color=BLUE, lw=2, marker="o", ms=5, mec=SURF, mew=1,
        label=f"vad_michelson, fixed  (bias {bias('new_p90'):+.0f} kt, MAE {mae('new_p90'):.0f} kt)")
...
order = [0, 3, 2, 1]
ax.legend([handles[i] for i in order], [labels[i] for i in order], loc="upper left", frameon=False, fontsize=9)
...
fig.text(0.03, 0.915,
         ...
         "Fixed Michelson is no longer biased low, but is noisier than vad_browning below 2.5 km.",
